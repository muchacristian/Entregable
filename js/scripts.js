...
const solicitarNumeroCandidatos = () => {
  return parseInt(
    prompt("Ingrese la cantidad de candidatos a participar en estas elecciones")
  );
};
  ...
  switch (voto) {
    ...
    default:
      if (voto >= 1 && voto <= candidatos.length) {
        ...
        alert(
          "¡Su voto ah sido registrador correctamente! \n Gracias por ejercer su derecho a votar."
        );
        votar(candidatos);
      } else {
...
const resultadoElecciones = (candidatos) => {
  ...
  alert(resultados);
  alert(
    `El nuevo Presidente de la Republica Argentina es ¡¡¡${ganadores[0].nombre}!!!`
  );
};
